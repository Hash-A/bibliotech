...
export async function getBooks(db, hint) {
  try {
    ...
    return fuse.search(hint).slice(0, 32).map(r => r.item);
  } catch (error) {
...
export async function downloadBook(db, bookId, url) {
  try {
    ...
    const extension = url.split('.').pop()?.split('?')[0] || 'txt';
    const fileUri = `${dir}book_${bookId}.${extension}`;
...
export async function getDownloadedBookContent(db, id) {
  try {
    ...
    if (!book.downloadPath.endsWith('.txt') && !book.downloadPath.endsWith('.html')) {
      console.warn(`Unsupported format for reading: ${book.downloadPath}`);
      return null;
    }
    ...
    console.error(`Failed to read downloaded book content (id: ${id}):`, error);
    return null;
  }
}
